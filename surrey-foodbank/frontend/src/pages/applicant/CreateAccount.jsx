...
//       <Typography variant="subtitle1" sx={{ mb: 1.5, color: "text.secondary", fontWeight: 600, fontSize: "1rem" }}>Your password must:</Typography>
//       <Stack spacing={1}>
...
function EmailFormatGuide({ t }) {
  ...
  return (
    <Paper elevation={0} sx={{ border: "1px solid #e0e0e0", borderRadius: 2, p: 1.25, mb: 1.5, backgroundColor: "#f5f7fa" }}>
      <Stack direction="row" alignItems="center" flexWrap="wrap" gap={1}>
        <Typography variant="caption" sx={{ color: "text.secondary", fontWeight: 600, whiteSpace: "nowrap", fontSize: "0.95rem" }}>{t.emailFormat}</Typography>
        <Stack direction="row" alignItems="center" flexWrap="wrap">
          {t.emailParts.map((part, i) => (
            <Tooltip key={i} title={<Box><Typography variant="caption" sx={{ fontWeight: 700, fontSize: "1.1rem" }}>{part.label}</Typography><Typography variant="caption" sx={{ display: "block", mt: 0.5, fontSize: "0.9rem" }}>{part.detail}</Typography></Box>} arrow placement="top" enterTouchDelay={0} slotProps={{
              tooltip: { sx: { backgroundColor: "primary.main" } },
              arrow: { sx: { color: "primary.main" } }
            }}>
              <Box component="span" sx={{ color: colors[i].color, backgroundColor: colors[i].bg, borderRadius: 0.75, px: 0.5, py: 0.1, fontFamily: "monospace", fontSize: "0.8rem", fontWeight: 700, cursor: "help", "&:hover": { opacity: 0.8 } }}>{part.text}</Box>
            </Tooltip>
          ))}
        </Stack>
        <Typography variant="caption" sx={{ color: "text.disabled", whiteSpace: "nowrap" }}>{t.hoverHelp}</Typography>
      </Stack>
    </Paper>
  );
}
...
function PasswordRequirementsChecklist({ value, t }) {
  ...
  return (
    <Paper elevation={0} sx={{ border: "1px solid #e0e0e0", borderRadius: 2, p: 1.5, mb: 1, backgroundColor: "#f5f7fa" }}>
      <Typography variant="subtitle1" sx={{ mb: 1.5, color: "text.secondary", fontWeight: 600, fontSize: "1rem" }}>{t.passwordMust}</Typography>
      <Stack spacing={1}>
        {PASSWORD_RULES.map((rule) => {
          const passed = rule.test(value);
          return (
...
function CreateAccount() {
  ...
  const emailField = useTextField("", (value) => {
    ...
    const users = JSON.parse(localStorage.getItem("users") || "[]");
    if (users.some((user) => user.email === value)) return t.emailExists;
    return "";
  });
  ...
  const handleSubmit = (e) => {
    ...
    const applicantKey = `applicant_${emailField.value}`;
    if (!localStorage.getItem(applicantKey)) {
      localStorage.setItem(applicantKey, JSON.stringify({
        email: emailField.value, firstName: "", lastName: "", name: "", phone: "",
        streetAddress: "", city: "", province: "", postalCode: "", address: "",
        statusInCanada: "", applyingToTinyBundles: "no", householdMembers: "",
        day: "", startTime: "", duration: 0, dateLabel: "", timeLabel: "",
      }));
    }
        ...
        <Paper elevation={3} sx={{ width: "100%", maxWidth: 1360, borderRadius: 4, overflow: "hidden", display: "grid", gridTemplateColumns: { xs: "1fr", md: "1fr 1fr" } }}>
          ...
          <Box sx={{ px: { xs: 4, md: 6 }, py: 4, display: "flex", flexDirection: "column", justifyContent: "center", alignItems: "center", textAlign: "center", backgroundColor: "#e8eef5" }}>
            ...
            <List disablePadding sx={{ width: "100%", maxWidth: 520 }}>
              <ListItem alignItems="flex-start" disableGutters>
                <ListItemIcon sx={{ minWidth: 40 }}><Typography sx={listItemIconStyle}>1.</Typography></ListItemIcon>
                <ListItemText primary={t.step1} secondary={<>{t.step1sub} <Link href="/applicant/login" color="primary">{t.step1subHere}</Link></>} slotProps={listItemTextStyle} />
              </ListItem>
              <ListItem alignItems="flex-start" disableGutters>
                <ListItemIcon sx={{ minWidth: 40 }}><Typography sx={listItemIconStyle}>2.</Typography></ListItemIcon>
              ...
              <ListItem alignItems="flex-start" disableGutters>
                <ListItemIcon sx={{ minWidth: 40 }}><Typography sx={listItemIconStyle}>4.</Typography></ListItemIcon>
                <ListItemText
                  primary={<>{t.step4} <Link href="https://maps.app.goo.gl/1H39wzvMBqmki2se6" color="primary">{t.step4link}</Link></>}
